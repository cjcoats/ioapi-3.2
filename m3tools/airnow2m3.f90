...
PROGRAM AIRNOW2M3
    ...
    !***********************************************************************
    ! Version "$Id: airnow2m3.f90 203 2021-10-14 18:02:11Z coats $"
    ...
    !
    !  DESCRIPTION:
    !       Read an ASCII  monitor-location file and a monitor-data file, and
    !       produce an I/O API monitor-data file.
    !
    !  PRECONDITIONS REQUIRED:
    !       Input ASCII files have agreed-upon AIRNOW format
    !
    !  SUBROUTINES AND FUNCTIONS CALLED:
    !       I/O API
    !       READHDR starts at line 536
    !       SKIPBLK starts at line 714
    !
    !  REVISION  HISTORY:
    !       10/1999:  Prototype by Carlie J. Coats, Jr., MCNC, for TNRCC data
    !       08/2000:  Version for EPA AIRNOW data also performs gridding--CJC
    !       09/2000:  Gridding functionality split out into program OBS_GRID--CJC
    !       03/2002:  Version for PAVE/I/O API distribution
    !
...
' ',                                                                    &
'Program version: ',                                                    &
'$Id:: airnow2m3.f90 203 2021-10-14 18:02:11Z coats                 $', &
' '

    LDEV = INIT3()
    IF ( .NOT.GETYN( 'Continue with program?', .TRUE. ) ) THEN
        CALL M3EXIT( PNAME, 0, 0, 'Terminated at user request.', 2 )
    END IF
    ...
    !...........  Count monitors in monitor-location file;
    !...........  copy header lines from monitor-location file and
    !...........  monitor-observation file to FDESC3 file header:
...
11  CONTINUE        !  loop counting monitors before allocation
    ...
    IF ( STATUS .NE. 0 ) THEN
        ...
        WRITE( MESG, '(A, I9, 2X, A, I9, 2X, A )' )     &
            'I/O error', STATUS, 'encountered at line', M, 'of MONITOR-LOCATION FILE'
...
21  CONTINUE        !  loop counting monitors before allocation
    ...
    IF ( STATUS .NE. 0 ) THEN
        ...
        WRITE( MESG, '(A, I9, 2X, A, I9, 2X, A )' )     &
            'I/O error', STATUS, 'encountered at line', M, 'of MONITOR-OBSERVATIONS FILE'
        CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
...
22  CONTINUE        !  end loop copying file header lines
    ...
    !...........  Read and sort ID's in monitor-location file:
...
70  CONTINUE        !  loop reading monitor locations
    ...
    IF ( STATUS .NE. 0 ) THEN
        ...
        WRITE( MESG, '(A, I9, 2X, A, I9, 2X, A )' )        &
            'I/O error', STATUS, 'encountered reading line', M, 'of MONITOR-LOCATION FILE'
        CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
    ...
    ELSE IF ( LINE( 1:1 ) .NE. '#' ) THEN
        ...
        II = STR2INT( LINE( 1:9 ) )
        IF ( II .LT. 0 ) THEN
            WRITE( MESG, '(3 A, I9, 2X, A)' ) 'Bad Monitor-ID "', BUFF( 1:11 ), '" at line', M, 'of LOC-FILE'
            CALL M3MSG2( MESG )
            EFLAG = .TRUE.
        ELSE
            MONU( N ) = II
        ...
        IF ( MIN( II,JJ,KK ) .LT. 0 ) THEN
            WRITE( MESG, '(3 A, I9, 2X, A)' ) 'Bad LAT "', LINE( 11:21 ), '" at line', M, 'of LOC-FILE'
            CALL M3MSG2( MESG )
            EFLAG = .TRUE.
        ELSE
        ...
        IF ( MIN( II,JJ,KK ) .LT. 0 ) THEN
            WRITE( MESG, '(3 A, I9, 2X, A)' ) 'Bad LON "', LINE( 23:33 ), '" at line', M, 'of LOC-FILE'
            CALL M3MSG2( MESG )
            EFLAG = .TRUE.
        ELSE
...
77  CONTINUE        !  end loop reading monitor locations
    ...
    IF ( EFLAG ) THEN
        CALL M3EXIT( PNAME, 0, 0, 'Error reading LOC-file', 2 )
    ELSE IF ( N .NE. NMONS ) THEN
        CALL M3EXIT( PNAME, 0, 0, 'Error counting IDs in LOC-file', 2 )
    END IF
    ...
    !...........  Read monitor-observation file:  first the header, then the data
...
90  CONTINUE        !  loop reading monitor data
    ...
    IF ( STATUS .NE. 0 ) THEN
        WRITE( MESG, '(A, I9, 2X, A, I9, 2X, A )' )        &
            'I/O error', STATUS, 'encountered reading line', M, 'of OBS-FILE'
        CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
    END IF
    ...
    IF ( LL .LT. 0 ) THEN
        WRITE( MESG, '(3 A, I9, 2X, A)' )        &
            'Bad Monitor-ID "', BUFF( 22:30 ), '" at line', M, 'of OBS-FILE'
        CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
    END IF
    ...
    IF ( N .LT. 1 ) THEN
        WRITE( MESG, '(A, I11, A, I9, 2X, A)' )        &
            'Monitor-ID "', LL, '" not found at line', M, 'of OBS-FILE'
        CALL M3MSG2( MESG )
        GO TO  90
    END IF

    READ( BUFF( 32:256 ), *, IOSTAT=STATUS ) ( O3 ( N,P ), P = 1, NHRS )
    IF ( STATUS .NE. 0 ) THEN
        WRITE( MESG, '(A, I9, 2X, A, I9, 2X, A )' )        &
            'I/O error', STATUS, 'reading monitor data at line', M, 'of OBS-FILE'
        CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
    END IF
...
99  CONTINUE        !  end loop reading monitor data
    ...
    IF ( EFLAG ) THEN
        CALL M3EXIT( PNAME, 0, 0, 'Error reading OBS-file', 2 )
    END IF
    ...
    DO  P = 1, NHRS
        ...
        IF ( .NOT.WRITE3( NCF_OBS, 'MONITOR_ID', JDATE, JTIME, MONS ) ) THEN
            MESG = 'Error writing variable MONITOR_ID'
            CALL M3EXIT( PNAME, JDATE, JTIME, MESG, 2 )
        END IF

        IF ( .NOT.WRITE3( NCF_OBS, 'LAT', JDATE, JTIME, LAT ) ) THEN
            MESG = 'Error writing variable '
            CALL M3EXIT( PNAME, JDATE, JTIME, MESG, 2 )
        END IF

        IF ( .NOT.WRITE3( NCF_OBS, 'LON', JDATE, JTIME, LON ) ) THEN
            MESG = 'Error writing variable '
            CALL M3EXIT( PNAME, JDATE, JTIME, MESG, 2 )
        END IF

        IF ( .NOT.WRITE3( NCF_OBS, 'O3', JDATE, JTIME, O3(1,P) ) ) THEN
            MESG = 'Error writing variable '
            CALL M3EXIT( PNAME, JDATE, JTIME, MESG, 2 )
        END IF
    ...
    CALL M3EXIT( PNAME, 0, 0, 'Successful completion of program AIRNOW2M3', 0 )
...
100     CONTINUE        !  loop reading file header
        ...
        IF ( STATUS .NE. 0 ) THEN
            ...
            WRITE( MESG, '(A, I9, 2X, A, I9, 2X, A )' )        &
                'I/O error', STATUS, 'reading line', M, 'of OBS-FILE'
        ...
        ELSE IF ( BUFF( 1:9 ) .EQ. 'START_DTG' ) THEN
            ...
            IF ( JJ .LT. 0 ) THEN
                WRITE( MESG, '(A, I9, 2X, A, 2X, A )' )        &
                     'Bad "START_DTG"  at line', M, 'of OBS-FILE:', BUFF( 1:40 )
            ...
            WRITE( MESG, '( A, I9, A, I6.6 )' ) 'Run starting date and time', SDATE, ':', STIME
            CALL M3MSG2( MESG )
            WRITE( MESG, '( A, I9, A, I6.6 )' ) 'File starting date and time', JDATE, ':', JTIME
            CALL M3MSG2( MESG )

            IF ( JDATE .NE. SDATE ) THEN
                WRITE( MESG, '(A, I9, 2X, A, 2X, A )' )        &
                     'Wrong date-field in "START_DTG"  at line', M, 'of OBS-FILE:', BUFF( 1:40 )
                CALL M3EXIT( RNAME, JDATE, JTIME, MESG, 2 )
            ELSE IF ( JJ .NE. 1 ) THEN
                WRITE( MESG, '(A, I9, 2X, A, 2X, A )' )        &
                     'Wrong time-field in "START_DTG"  at line', M, 'of OBS-FILE:', BUFF( 1:40 )
                CALL M3EXIT( RNAME, JDATE, JTIME, MESG, 2 )
            END IF
        ...
        ELSE IF ( BUFF( 1:8 ) .EQ. 'INTERVAL' ) THEN
            ...
            IF ( TSTEP .NE. 60 ) THEN
                WRITE( MESG, '(A, I9, 2X, A, 2X, A )' )        &
                     'Bad "INTERVAL"  at line', M, 'of OBS-FILE:', BUFF( 1:40 )
                CALL M3EXIT( RNAME, 0, 0, MESG, 2 )
            END IF
        ...
        ELSE IF ( BUFF( 1:8 ) .EQ. 'NUMSTEPS' ) THEN
            ...
            IF ( NUMT .LT. 0 ) THEN
                WRITE( MESG, '(A, I9, 2X, A, 2X, A )' )        &
                     'Bad "NUMSTEPS"  at line', M, 'of OBS-FILE:', BUFF( 1:40 )
                CALL M3EXIT( RNAME, 0,0,MESG,2 )
            ELSE IF ( NUMT .LT. NHRS ) THEN
                WRITE( MESG, '(A, I9, 2X, A, 2X, A )' )        &
                     'Too-small "NUMSTEPS"  at line', M, 'of OBS-FILE:', BUFF( 1:40 )
                CALL M3EXIT( RNAME, 0, 0, MESG, 2 )
            END IF
            WRITE( MESG, '(A, I4 )' ) 'Number of input time steps', NUMT
            CALL M3MSG2( MESG )
        ...
        ELSE IF ( BUFF( 1:8 ) .EQ. 'AVG_TIME' ) THEN
            ...
            IF ( AVGT .NE. 60 ) THEN
                WRITE( MESG, '(A, I9, 2X, A, 2X, A )' )        &
                     'Bad "AVG_TIME"  at line', M, 'of OBS-FILE:', BUFF( 1:40 )
                CALL M3EXIT( RNAME, 0, 0, MESG, 2 )
            END IF
        ...
        ELSE IF ( BUFF( 1:8 ) .EQ. 'STATIONS' ) THEN
            ...
            IF ( JJ .LT. 0 ) THEN
                WRITE( MESG, '(A, I9, 2X, A, 2X, A )' )        &
                     'Bad "STATIONS"  at line', M, 'of OBS-FILE:', BUFF( 1:40 )
                CALL M3EXIT( RNAME, 0, 0, MESG, 2 )
            ELSE IF ( JJ .GT. NMONS ) THEN
                WRITE( MESG, '(A, I9, 2X, A, 2X, A )' )        &
                     'Too-big "STATIONS"  at line', M, 'of OBS-FILE:', BUFF( 1:40 )
                CALL M3EXIT( RNAME, 0, 0, MESG, 2 )
            END IF
