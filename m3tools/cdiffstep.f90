...
SUBROUTINE CDIFFSTEP ( NCELL,  NLAYS,  NVARS,               &
    ...
    !***********************************************************************
    ! Version $Id: cdiffstep.F 1703 2013-11-15 21:39:36Z coats@bdsl$"
    ...
    !
    !  REVISION  HISTORY:
    !       Prototype 3/1999 by CJC, adapted for non-gridded data from diffstep.F
    !
    !       Modified  9/1999 by CJC for enhanced portability
    !
    !       Version  11/2007 by CJC: REAL*8 accumulators
    !
    ...
    !***********************************************************************
    !   begin body of subroutine  CDIFFSTEP
    ...
    STATFMT = "(1X,A3,2(1X,1PE12.5,'@(',I1,',',I1,')',),2(1X,1PE12.5))"
    WRITE( STATFMT(27:27), '(I1)' ) NCD
    WRITE( STATFMT(34:34), '(I1)' ) NLD

    NCD = NCD-1
    NLD = NLD-1
    LGNDFMT = "(6X,'MAX        @(', X, 'C,', X,'L)  '" //&
                  "'Min        @(', X, 'C,', X,'L)  '" //&
                  "'Mean         Sigma', A)'"
    ...
    IF ( JDATEA .NE. 0  .OR. JTIMEA .NE. 0 ) THEN
        ...
        IF ( JDATEB .EQ. 0  .AND. JTIMEB .EQ. 0 ) THEN
            WRITE( RDEV,92010 ) 'Date and time', JDATEA, JTIMEA, DTBUFA
    ...
    DO  399  V = 1, NVARS
        ...
        ELSE
            CALL M3WARN( 'M3DIFF:DIFFSTEP', JDATEA, JTIMEA,     &
                         'Read failure:  file ' // NAMEA // ' variable ' // WNAMES( 1,V ) )
        END IF
        ...
        ELSE
            CALL M3WARN ( 'M3DIFF:DIFFSTEP', JDATEA, JTIMEA,&
                         'Read failure:  file ' // NAMEB // ' variable ' // WNAMES( 2,V ) )
        END IF

        IF ( FLAG1 .AND. FLAG2 ) THEN

            CALL NAMEDOP( OPNAME( V ), NCELL, 1, 1, NLAYS, GRID1, GRID2, GRID3 )
            IF ( NAMEC( 1:5 ) .NE. 'NONE ' ) THEN
                IF ( .NOT. WRITE3( NAMEC, VNAME( V ), JDATEC, JTIMEC, GRID3 ) ) THEN
                    MESG = 'Could not write "' //               &
                        TRIM( VNAME( V ) ) //  '" to "' //      &
                        TRIM( NAMEC )      //  '" for ' // DT2STR( JDATEC, JTIMEC )
                    CALL M3MESG( MESG )
                END IF
            END IF
