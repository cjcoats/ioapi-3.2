...
PROGRAM  M3AGMASK
    ...
    !***********************************************************************
    ! Version "$Id: m3agmask.f90 203 2021-10-14 18:02:11Z coats $"
    ...
    !.........................................................................
    !  program body starts at line 13
    !
    !  FUNCTION:
    ...
    !       Computes statistics for a user-selected set of mask regions
    !       (defined by GRID==1 in a set of user-input mask files).
    !       Logs sorted list of max values, their locations, and times.
    !
    ...
    !       GETNUM, NEXTIME, Models-3 I/O.
    !
    !  REVISION  HISTORY:
    !       Prototype 5/1997 by M Houyoux
    !
    !       Modified 11/1999 by Carlie J. Coats, Jr.:  max-value report
    !
    ...
    !.........................................................................
    !   begin body of program  M3AGMASK

    LOGDEV = INIT3()
    WRITE ( *,92000 ) ' ', BAR,                                         &
'Program M3AGMASK to sum, average, or find the maximum values over a',  &
'repeating time period from a selected time window.',                   &
'The time period and starting time window set the start and duration',  &
'of all subsequent time windows.',                                      &
' ',                                                                    &
'This version of the program will also report to the program log a',    &
'sorted list of the hourly maxes of the output, taken over a set of',   &
'selected subregions, by variable, together with their times and',      &
'locations.  The subregions are defined by user-input "mask" files,',   &
'each of which is defined by an input INTEGER grid of zeros and ones,', &
'the region being specified as the locus where the mask-grid value is 1.',&
'The program will prompt you for the set of files used to define these',&
'subregions, and the names of the mask variables within those files.',  &
' ',                                                                    &
'You need to have set environment variables for the input and',         &
'output file logical names.  You will be asked to select the',          &
'time period to be copied and the start of the time period',            &
...
' ',                                                                    &
'Program version: ',                                                    &
'$Id:: m3agmask.f90 203 2021-10-14 18:02:11Z coats                   $',&
' '
    ...
    ELSE
        ...
        CALL M3EXIT( PNAME, 0, 0, 'usage:  M3AGMASK [INFILE OUTFILE MAXFILE ]', 2 )
    ...
    IF ( FTYPE3D .EQ. GRDDED3 ) THEN
        ...
        NLAYS = NLAYS3D
    ELSE
        WRITE( MESG, 94011 ) 'Input file "', TRIM( INAME ), '" has unsupported type', FTYPE3D
        CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
    END IF
...
11  CONTINUE
    ...
    NPFLAG = ENVYN( 'M3AGMASK_ALLV', 'true if no prompting for variables to output', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMASK_ALLV"', 2 )
    END IF

    !.......  If no prompting set to total number of vars, or prompt
...
122     CONTINUE
        ALINE = 'Enter output-name for this variable [' // TRIM( VNAME3D( V ) ) // '] >> '
...
133     CONTINUE        !  end loop getting variables-list for analysis
    ...
    IF ( TSTEP .EQ. 0 ) THEN        !  time-independent file
        ...
        MESG = 'Input file "' // TRIM( INAME ) // '" is only one time step- no output written.'
        CALL M3EXIT( PNAME, 0, 0, MESG, 2 )

    ELSE                            !  time-dependent file

    !.......   Prompt for parameters if defaults aren't set by specific env.
    !.......   variable setting. Get default settings from the environment.

    !...........  Period length, Start date&time BDATE:BTIME, duration WINLEN
    !...........  Number NOUTS of periods to process, analysis type ATYPE

        PERIOD = ENVINT( 'M3AGMASK_PLEN', 'Output time step', TSTEP, IOS )
        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMASK_PLEN"', 2 )
        END IF
        ...
        BDATE = ENVINT( 'M3AGMASK_BDATE', 'Window start date', SDATE, IOS )

        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMASK_BDATE"', 2 )
        ...
        BTIME = ENVINT( 'M3AGMASK_BTIME', 'Window start time', STIME, IOS )

        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMASK_BTIME"', 2 )
        ELSE IF( IOS .NE. 0 )  THEN
            I = BTIME
            BTIME  = GETNUM( 0, 239999, I, 'Enter starting time for run (HHMMSS)' )
        END IF
        ...
        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMASK_WLEN"', 2 )
        ELSE IF( IOS .NE. 0 ) THEN
            I = WINLEN
            WINLEN = GETNUM( 1, T, I, 'Enter duration of aggregation-period (HHMMSS)' )
        END IF
        ...
        IF( IOS .NE. 0 )  THEN
            I = NOUTS
            NOUTS = GETNUM( 1, I, T, 'Enter number of output time steps to analyze' )
        END IF
        ...
        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMASK_TYPE"', 2 )
        ELSE IF( IOS .NE. 0 ) THEN
            ATYPE = GETMENU( MAXITEMS, ATYPE, 'Enter type of operation to perform', MENUITMS )
        END IF
    ...
    IF ( ARGCNT .EQ. 0 ) THEN
        ...
        DO   V = 1, NVARS
            ...
            VTYPE3D( V ) = VTYPE ( V )
        END DO
        ONAME = PROMPTMFILE( 'Enter logical name for AGG FILE', FSUNKN3, 'OUTFILE', PNAME )
        ...
        MNAME = PROMPTMFILE( 'Enter logical name for AGG-MAX FILE', FSUNKN3, 'MAXFILE', PNAME )
    ...
    ELSE        !  argcnt = 3:
        ...
        IF ( .NOT. OPEN3( ONAME, FSUNKN3, PNAME ) ) THEN
            MESG = 'Could not open output AGG-file "' // TRIM( ONAME ) // '"'
        ...
        IF ( .NOT. OPEN3( MNAME, FSUNKN3, PNAME ) ) THEN
            MESG = 'Could not open output AGG-MAX file "' // TRIM( MNAME ) // '"'
            CALL M3EXIT( PNAME, SDATE, STIME, MESG, 2 )
        END IF
    ...
    !.......   Count/Get the mask files and mask variables:

    NMASKS = 0
144 CONTINUE

    WRITE( ANAME, '( A, I3.3 )' ) 'MASKFILE', I
    BNAME = PROMPTMFILE( 'Enter the name for the next MASK FILE, or "NONE"', FSREAD3, ANAME, PNAME )
    IF ( BNAME .NE. 'NONE ' ) THEN
        NMASKS = NMASKS + 1
        MASKS( NMASKS ) = BNAME

        IF ( .NOT. DESC3( BNAME ) ) THEN
            MESG = 'Could not get file description for "' // TRIM( BNAME ) // '"'
            CALL M3EXIT( PNAME, SDATE, STIME, MESG, 2 )
        END IF
        MASKV( NMASKS ) = VNAME3D( GETMENU( NVARS3D, 1, 'Enter number for the MASK VARIABLE', VNAME3D ) )
        GO TO  144
    END IF

    !.......   Allocate masked-aggregation arrays:
    ...
    WRITE( MESG, '(A, I9, A, I6.6)' ) 'Starting date and time:', BDATE, ':', BTIME
    CALL M3MSG2( MESG )

    WRITE( MESG, '(A, 2X, I6.6)' ) 'Output timestep (HHMMSS)', PERIOD
    CALL M3MSG2( MESG )
    ...
    !...........   Sort the aggregate-maxes:
        ...
        DO  M = 1, NMASKS
            ...
            DO  I = NOUTS, 1, -1
                ...
                T = CDEX( I,M )
                CBAR = CBAR + CMAX( T,M )
                WRITE( LOGDEV,'( 5X,A,2X,I4,2X,A,2X,1PE14.7,2X,A,I7.7,A,I6.6,2X,A,3( I3,A ) )' )&
                     'Rank:', I,&
                     'Max:', CMAX( T,M ),&
