...
PROGRAM  M3AGMAX
    ...
    !***********************************************************************
    ! Version "$Id: m3agmax.f90 212 2021-11-10 20:39:53Z coats $"
    ...
    !.........................................................................
    !  program body starts at line 130
    !
    !  FUNCTION:
    ...
    CHARACTER*64, PARAMETER :: MENUITMS( MAXITEMS ) =   &   !!  buffer for operations menu items
    ...
    !.........................................................................
    !   begin body of program  M3AGMAX

    LOGDEV = INIT3()
    WRITE ( *,92000 ) ' ', BAR,&
'Program M3AGMAX to sum, average, or find the maximum values over a',   &
...
'Models-3 I/O API files.',                                              &
' ',                                                                    &
'This version of the program will also report to the program log',      &
'a sorted list of the hourly maxes of the output, by  variable, ',      &
'together with their times and locations.',                             &
' ',                                                                    &
'You need to have set environment variables for the input and output',  &
'file logical names.  You will be asked to select the time period',     &
'to be copied and the start of the time period to receive the results.',&
' ',                                                                    &
...
' ',                                                                    &
'Program version: ',                                                    &
'$Id:: m3agmax.f90 212 2021-11-10 20:39:53Z coats                    $',&
' '

    ARGCNT = IARGC()

    IF ( ARGCNT .NE. 0  .AND.  ARGCNT .NE. 3 ) THEN
        CALL M3EXIT( PNAME, 0, 0, 'usage:  M3AGMAX [INFILE OUTFILE MAXFILE ]', 2 )
    END IF
...
11  CONTINUE
    ...
    NPFLAG = ENVYN( 'M3AGMAX_ALLV', 'true if no prompting for variables to output', .FALSE., IOS )
    IF ( IOS .GT. 0 ) THEN
        CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMAX_ALLV"', 2 )
    END IF
...
199     CONTINUE        !  end loop getting variables-list for analysis
    ...
    ELSE                            !  time-dependent file
        ...
        PERIOD = ENVINT( 'M3AGMAX_PLEN', 'Output time step', TSTEP, IOS )
        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMASK_PLEN"', 2 )
        ELSE IF( IOS .NE. 0 ) THEN
            PERIOD = GETNUM( TSTEP, RUNLEN, TSTEP, 'Enter output time step (HHMMSS) for repeating analysis' )
        END IF
        ...
        BDATE = ENVINT( 'M3AGMAX_BDATE', 'Window start date', SDATE, IOS )

        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMAX_BDATE"', 2 )
        ELSE IF( IOS .NE. 0 ) THEN
        ...
        BTIME = ENVINT( 'M3AGMAX_BTIME', 'Window start time', STIME, IOS )

        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMAX_BTIME"', 2 )
        ELSE IF( IOS .NE. 0 )  THEN
            I = BTIME
        ...
        WINLEN = ENVINT( 'M3AGMAX_WLEN', 'Aggregation-period duration', I, IOS )

        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMAX_WLEN"', 2 )
        ELSE IF( IOS .NE. 0 ) THEN
            I = WINLEN
        ...
        NOUTS = ENVINT( 'M3AGMAX_NPER', 'Number of output time steps', RUNSEC / PERSEC, IOS )

        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMAX_NPER"', 2 )
        ELSE IF( IOS .NE. 0 )  THEN
            I = NOUTS
        ...
        ATYPE = ENVINT( 'M3AGMAX_TYPE', 'Type of analysis', M3AVE, IOS )

        IF ( IOS .GT. 0 ) THEN
            CALL M3EXIT( PNAME, 0,0, 'Bad environment variable "M3AGMAX_TYPE"', 2 )
        ELSE IF( IOS .NE. 0 ) THEN
    ...
    ALLOCATE ( CMAX ( RUNLEN ),     &
               ...
               CDEX ( RUNLEN ), STAT = I )
    IF ( I .NE. 0 ) THEN
        WRITE( MESG, '(A, I9)' )  'Allocation failure:  STATUS=', I
        CALL M3EXIT( PNAME, SDATE, STIME, MESG, 2 )
    END IF
...
322 CONTINUE            !  end loop on variables
    ...
    CALL M3EXIT( PNAME, 0, 0, 'Program  M3AGMAX  completed successfully', 0 )
